  ...
  <basic-container>
    ...
    >
      <el-form ref="form" :model="form" label-width="90px" class="form-footer">
        <el-form-item label="单据编号:" prop="assetsDisposalSn">
          <el-input
            placeholder="请输入"
            v-model="form.assetsDisposalSn"
          ></el-input>
        </el-form-item>
        <el-form-item label="处置方式:" prop="branch">
          <el-select v-model="form.disposalWay" placeholder="请选择">
            <el-option
    ...
    <el-row class="operation-function">
      <el-button
        v-if="permission.equipmentDisposal_expore"
        size="small"
        icon="el-icon-search"
        type="primary"
      ...
      <el-button
        v-if="permission.equipmentDisposal_add"
        size="small"
        icon="el-icon-plus"
        type="primary"
        @click="handleAdd"
    ...
    <el-row>
      ...
      >
        ...
        <el-table-column prop="assetsDisposalSn" label="处置编号">
        </el-table-column>
        <el-table-column prop="createTime" label="处置日期"> </el-table-column>
        <el-table-column prop="disposalWay" label="处置方式"> </el-table-column>
        <el-table-column prop="disposalPerson" label="处置人">
        </el-table-column>
        <el-table-column prop="remark" label="备注"> </el-table-column>
        <el-table-column width="130" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-tooltip
              v-if="permission.equipmentDisposal_audit"
              class="item"
              effect="dark"
              content="提交审核"
              placement="bottom"
            ...
            <el-tooltip
              v-if="permission.equipmentDisposal_edit"
              class="item"
              effect="dark"
              content="编辑"
              placement="bottom"
            ...
            <el-divider direction="vertical"></el-divider>
            <el-tooltip
              v-if="permission.equipmentDisposal_detail"
              class="item"
              effect="dark"
              content="详情"
              placement="bottom"
            ...
            <el-divider direction="vertical"></el-divider>
            <el-tooltip
              v-if="permission.equipmentDisposal_delete"
              class="item"
              effect="dark"
              content="删除"
              placement="bottom"
...
export default {
  ...
  data() {
    return {
      ...
      deleteApi: deleteAssetsDisposalBatch,
      auditApi: audit,
      options: [],
    };
  },
  ...
  methods: {
    ...
    handleAdd() {
      // 新增
      this.$router.push({
        path: "/asset-management/equipmentDisposal/add",
      });
    },
    handleEdit(row) {
      // 修改
      this.$router.push({
        path: "/asset-management/equipmentDisposal/edit",
        query: {
          data: row,
        },
      });
    ...
    handleDetail(row) {
      // 详情
      this.$router.push({
        path: "/asset-management/equipmentDisposal/detail",
        query: {
          data: row,
        },
      });
