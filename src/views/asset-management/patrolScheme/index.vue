...
<template>
  <basic-container>
    ...
    <el-row class="operation-function">
      <el-button
        v-if="permission.patrolScheme_expore"
        size="small"
        icon="el-icon-search"
        type="primary"
      ...
      <el-button
        v-if="permission.patrolScheme_set"
        size="small"
    ...
    <el-row>
      ...
      >
        ...
        <el-table-column prop="spotCheckOrderSn" label="计划单号" width="150px">
        </el-table-column>
        ...
        <el-table-column width="160" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-tooltip
              v-if="permission.patrolScheme_audit"
              class="item"
              effect="dark"
              content="提交审核"
              placement="bottom"
            ...
            <el-divider direction="vertical"></el-divider>
            <el-tooltip
              v-if="permission.patrolScheme_detail"
              class="item"
              effect="dark"
              content="详情"
              placement="bottom"
...
import { updateStatusSpotCheck } from "@/api/asset-management/checkingPlan.js";
...
export default {
  ...
  methods: {
    checkSet() {
      this.$router.push({
        path: "/asset-management/patrolScheme/set",
      });
    },
    //查询
    onSearch() {
    ...
    handleAdd() {
      // 新增
      this.$router.push({
        path: "/asset-management/patrolScheme/add",
      });
    },
    handleEdit(row) {
      // 修改
      this.$router.push({
        path: "/asset-management/patrolScheme/edit",
        query: {
          data: row,
        },
      });
    ...
    handleDetail(data) {
      ...
      console.log(data);
      this.$router.push({
        path: "/asset-management/patrolScheme/detail",
        query: {
          data,
        },
      });
