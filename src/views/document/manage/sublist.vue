        ...
        <el-table-column prop="folderName" label="文件名">
          <template slot-scope="scope">
            ...
            <span
              ...
              @click="handleFolderName(scope.row)"
              class="foldertxt"
              >{{ scope.row.folderName }}</span
            >
            <span v-else>{{ scope.row.folderName }}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      folderManagerReq: {
        folderName: "",
        id: "",
        superiorId: this.$route.query.id, //上级id
      },
      tableData: [],
      multipleSelection: [],
  ...
  methods: {
    //
    handleFolderName(info) {
      this.$router.push({
        path: "/document/manage/sublist",
        query: { id: info.id, name: info.folderName },
      });
    },
    //新增
    handleAdd(type, info) {
    ...
    reset() {
      this.folderManagerReq = {
        superiorId: this.$route.query.id,
      };
    ...
    async onSumbit() {
      if (this.iFolderManagerReq.folderName) {
        (this.iFolderManagerReq.superiorId = this.$route.query.id),
          (this.iFolderManagerReq.superiorName = this.$route.query.name);
