    ...
    <el-form label-width="180px">
      ...
      <el-row class="footer-box margin_30">
        ...
        <el-button
          ...
          >驳回</el-button
        >
        <el-button @click="$router.push('/flow2/todoList')" size="small"
          >取消</el-button
        >
      </el-row>
      <el-dialog
...
<script>
import { listDetail, auditApproval } from "@/api/flow2/todoList.js";
...
export default {
  ...
  data() {
    return {
      form: {
        approvalNodeSelectResp: [{ roleName: "" }],
      },
      requesting: false,
      info: {},
      tableData: [],
  ...
  methods: {
    ...
    async handle(e) {
      let s = e == 0 ? "同意" : "拒绝";
      try {
        await this.$confirm("确认" + s + "该申请吗？", "确认", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        });
        let params = {
          projectProcessId: this.form.id,
          approval: e,
        };
        const res = await auditApproval(params);
        if (res.data.code === 200) {
