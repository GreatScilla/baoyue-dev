  ...
  <basic-container>
    ...
    >
      <el-row>
        <el-col :span="12">
          ...
          <el-form-item required label="项目:">
            <span v-if="type === 'detail'">
              {{ incomeStatementForm.organName }}</span
            >
            <el-input
              v-else
              @click.native="projectCustomerDialogVisible = true"
              suffix-icon="el-icon-search"
              size="small"
              v-model="incomeStatementForm.projectName"
              placeholder="请选择项目名称"
              :disabled="disabled"
            >
            </el-input>
          </el-form-item>
          <el-form-item label="应收金额:" prop="roleName">
            <span v-if="type === 'detail'">
      ...
      >
        ...
        <el-table-column label="收款金额" align="center">
          <template slot="header" slot-scope="scope">
            <span style="color: #f56c6c">*</span>收款金额
          </template>
          <template slot-scope="scope">
    ...
    <el-row class="button-wrapper-footer">
      <el-button
        v-if="!examine"
        size="small"
        @click="$router.push('/financial/incomeStatement')"
        >取消</el-button
      >
      <el-button
        v-if="type !== 'detail'"
    ...
    <project-customer
      title="项目"
      :visible.sync="projectCustomerDialogVisible"
...
import projectCustomer from "@/components/dialog-select/projectCustomer";
...
export default {
  ...
  data() {
    return {
      projectCustomerDialogVisible: false, //选择项目对话框
      incomeStatementForm: {
        amount: 0,
  ...
  methods: {
    ...
    onSelectedContactUnit(customer, order) {
      ...
      if (order.length > 0) {
        ...
        this.incomeStatementForm.projectName = customer.projectName;
        this.incomeStatementForm.projectSn = customer.projectSn;
        this.incomeStatementForm.organName = customer.customerName;
        this.incomeStatementForm.organId = customer.customerId;
      }
      this.projectCustomerDialogVisible = false;
    },
    // 选择人员
    ...
    async saveIncomeStatement() {
      console.log(this.incomeStatementForm);
