  ...
  <basic-container>
    ...
    <el-row class="operation-function">
      <el-button
        ...
        v-if="permission.otherPayable_search"
        >查 询</el-button
      >
      <el-button
        size="small"
        icon="el-icon-plus"
        type="primary"
        @click="$router.push('/financial/otherPayable/add')"
        v-if="permission.otherPayable_add"
        >新 增</el-button
      >
      <!-- <el-button
        size="small"
        ...
        plain
        @click="examine"
        v-if="permission.otherPayable_examine"
        >批量提交审核</el-button
      > -->
      <el-button
        size="small"
        type="primary"
        @click="transAll"
        v-if="permission.otherPayable_trans"
        >转总账</el-button
      >
      <el-button
        size="small"
        ...
        >批量删除</el-button
      >
      <!-- <el-button size="small" icon="el-icon-upload2" type="success" plain v-if="permission.otherPayable_export">导 出</el-button>
      <el-button size="small" icon="el-icon-printer" type="primary" plain v-if="permission.otherPayable_print">打 印</el-button> -->
    </el-row>
    <el-divider></el-divider>
    <!-- 表格table -->
    <el-table
      @selection-change="handleSelectionChange"
      :data="otherPayableListData"
      border
    >
      ...
      <el-table-column width="150" label="操作" fixed="right">
        <template slot-scope="scope">
          ...
          <el-divider
            direction="vertical"
            v-if="
              permission.otherPayable_examine && permission.otherPayable_edit
            "
          ></el-divider>
          <el-tooltip
            class="item"
          ...
          >
            <el-button
              ...
              v-if="permission.otherPayable_edit"
            >
            </el-button>
          </el-tooltip>
          <el-divider
            direction="vertical"
            v-if="
              permission.otherPayable_edit && permission.otherPayable_detail
            "
          ></el-divider>
          <el-tooltip
            class="item"
          ...
          >
            <el-button
              ...
              v-if="permission.otherPayable_detail"
            >
            </el-button>
          </el-tooltip>
          <el-divider
            direction="vertical"
            v-if="
              permission.otherPayable_detail && permission.otherPayable_delete
            "
          ></el-divider>
          <el-tooltip
            class="item"
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "修改其他应付" || from.name === "其他应付详情") {
  ...
  methods: {
    ...
    async handleList(...arg) {
      ...
      this.otherPayableListData = res.data.data.records;
      this.total = res.data.data.total;
    },
    async mouseenter(id) {
      this.otherPayableListData.map((item, index) => {
        if (id == item.id) {
          console.log(id);
          item.showIndex = true;
          detailsIncomeAndExpenditure({ id: id }).then((res) => {
            this.otherPayableListData[index].billAccountAmountList =
    ...
    mouseLeave() {
      this.otherPayableListData.map((item) => {
        item.showIndex = false;
      });
    },
    // 查询
    ...
    toEdit(row) {
      this.$router.push({
        path: "/financial/otherPayable/edit",
        query: {
          data: row,
        },
      });
    ...
    toDetail(row) {
      this.$router.push({
        path: "/financial/otherPayable/detail",
        query: {
          data: row,
        },
      });
