  ...
  <basic-container>
    ...
    <el-row class="operation-function">
      <el-button
        ...
        v-if="permission.expenseList_search"
        >查 询</el-button
      >
      <el-button
        size="small"
        icon="el-icon-plus"
        type="primary"
        @click="$router.push('/financial/expenseList/add')"
        v-if="permission.expenseList_add"
        >新 增</el-button
      >
      <el-button
        size="small"
        type="primary"
        @click="transAll"
        v-if="permission.expenseList_trans"
        >转总账</el-button
      >
      <!-- <el-button
        size="small"
        ...
        plain
        @click="examine"
        v-if="permission.expenseList_examine"
        >批量提交审核</el-button
      > -->
      <el-button
        size="small"
        ...
        v-if="permission.expenseList_delete"
        >批量删除</el-button
      >
            <!-- <el-button
        size="small"
        ...
        v-if="permission.expenseList_print"
        >导出</el-button
      > -->
      <!-- <el-button
        size="small"
        ...
        plain
        @click="handleDelInboundManage"
        v-if="permission.expenseList_export"
        >导出</el-button
      > -->
      <!-- <el-button size="small" icon="el-icon-upload2" type="success" plain v-if="permission.expenseList_export">导 出</el-button>
      <el-button size="small" icon="el-icon-printer" type="primary" plain v-if="permission.expenseList_print">打 印</el-button> -->
    </el-row>
    ...
    >
      ...
      <!-- <el-table-column prop="transStatus" label="转总账状态">
        <template slot-scope="scope">
          {{ scope.row.transStatus | billStatus }}
        </template>
      </el-table-column> -->
      ...
      <el-table-column prop="organName" label="供应商名称"></el-table-column>
      ...
      <el-table-column width="180" label="操作" fixed="right">
        <template slot-scope="scope">
          <span v-if="permission.expenseList_examine">
            <el-tooltip
              class="item"
              effect="dark"
              content="提交审核"
          ...
          <span v-if="permission.expenseList_edit">
            <el-tooltip
              class="item"
              effect="dark"
              content="编辑"
          ...
          <span v-if="permission.expenseList_detail">
            <el-tooltip
              class="item"
              effect="dark"
              content="详情"
          ...
          <span v-if="permission.expenseList_delete">
            <el-tooltip
              class="item"
              effect="dark"
              content="删除"
...
export default {
  ...
  data() {
    return {
      ...
      searchForm: {},
      recordForm: {}, //暂存数据
      pdfIdApi: "/finance/bill/getExpendBillsReportPDF",
      incomeListData: [],
      ids: [],
      tableRowData: [],
      billDate: [],
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "修改支出管理" || from.name === "支出管理详情") {
  ...
  async created() {
    const res3 = await getFullList({ size: 9999 });
    this.paymentAccountOptions = res3.data.data.records;
  ...
  methods: {
      ...
      this.incomeListData.map((item, index) => {
        if (id == item.id) {
          ...
          detailsIncomeAndExpenditure({ id: id }).then((res) => {
            this.paymentAccountOptions.map((el) => {
              res.data.data.billAccountAmountList.map((item) => {
                if (el.id == item.payAccountId) {
                  item.payAccount = el.openAccountName || "现金";
                }
              });
            });
    ...
    async examine(row) {
      ...
      await this.$confirm("确定该支出单提交审核吗?", "提示", {
    ...
    async handlePrinter(reportName) {
      ...
      if (flag.length <= 0 || flag.length != this.tableRowData.length) {
        this.$message.warning("请选择已审核的数据打印");
        return;
      }
      ids = this.tableRowData.map((ele) => ele.id);
      try {
        // 减少服务器压力
        await this.$confirm("确定打印单据吗?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        });
    ...
    async handleDelInboundManage(row) {
      ...
      await this.$confirm("确定要删除该支出单吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      });
    ...
    toEdit(row) {
      this.$router.push({
        path: "/financial/expenseList/edit",
        query: {
          data: row,
        },
      });
    ...
    toDetail(row) {
      this.$router.push({
        path: "/financial/expenseList/detail",
        query: {
          data: row,
        },
      });
