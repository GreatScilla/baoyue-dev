          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                icon="el-icon-view"
                :disabled="!(scope.row.status == 2)"
                @click="getPdfPreview(pdfIdApi, scope.row)"
              ></el-button>
            </el-tooltip>
            <!-- <el-divider direction="vertical"></el-divider>
            <el-tooltip
...
export default {
  ...
  data() {
    return {
      ...
      fileId: "",
      pdfShow: false,
      pdfIdApi: getPricingBillsReportPDF,
      form: {
        pricingSn: "",
        supplierName: "",
        startTime: "",
  ...
  methods: {
    async getPdfPreview(pdfIdApi, row) {
      this.fileId = row.id;
      const res = await pdfIdApi({ id: row.id });
      console.log(res);
