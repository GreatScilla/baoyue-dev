...
<template>
  <basic-container>
    ...
    <el-row style="margin-bottom: 20px">
      <el-button
        v-if="permission.feedstock_check_expore"
        size="small"
        icon="el-icon-search"
        type="primary"
      ...
      <!-- <el-button
        v-if="permission.feedstock_check_aduit"
        size="small"
      ...
      > -->
      <el-button
        v-if="permission.feedstock_check_delete"
        size="small"
        icon="el-icon-delete"
        type="danger"
        plain
      ...
      >
      <!-- <el-button
        v-if="permission.feedstock_check_printer"
        size="small"
        icon="el-icon-printer"
        plain
        @click="handlePrinter('PuechaseCheckOrderV1.3.1进料检验单.ureport')"
        >导出</el-button
      > -->
    </el-row>
    <el-row>
      ...
      >
        ...
        <el-table-column prop="procCode" label="单据状态" width="90">
            ...
            <el-tooltip>
              <div slot="content" style="font-size: 14px">
                ...
                >
                  ...
                  <tr
                    bgcolor="#303133"
                    v-for="(item, index) in scope.row.purchaseCheckItemList"
                    :key="index"
                  >
        ...
        <el-table-column width="190px" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-tooltip
              v-if="permission.feedstock_check_aduit"
              class="item"
            ...
            <span v-if="permission.feedstock_check_edit">
              <el-tooltip
                class="item"
                effect="dark"
                content="编辑"
            ...
            <el-tooltip
              v-if="permission.feedstock_check_detail"
              class="item"
              effect="dark"
              content="详情"
              placement="bottom"
            ...
            <el-divider direction="vertical"></el-divider>
            <span v-if="permission.feedstock_check_delete">
              <el-tooltip
                class="item"
                effect="dark"
                content="删除"
...
let recordObj = "";
import {
  getPurchaseCheckByPage,
  deleteApi,
  audit,
} from "@/api/purchase/feedstockCheck";
...
export default {
  ...
  data() {
    return {
      pdfIdApi: "/erp/purchaseCheck/getPurchaseCheckBillsReportPDF",
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "检验" || from.name === "检验详情") {
  ...
  methods: {
    ...
    async handleList() {
      ...
      const res = await getPurchaseCheckByPage(params);
    ...
    handleDetail(row) {
      this.$router.push({
        path: "/purchaseManage/feedstockCheck/checkDetail",
        query: {
          data: row,
        },
      });
    ...
    handleEdit(row) {
      this.$router.push({
        path: "/purchaseManage/feedstockCheck/check",
        query: {
          data: row,
        },
      });
