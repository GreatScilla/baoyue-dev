...
<template>
  <basic-container>
    ...
    <el-row class="operation-function">
      <el-button
        v-if="permission.purchaseStatement_expore"
        size="small"
        icon="el-icon-search"
        type="primary"
      ...
      >
      <el-button
        v-if="permission.purchaseStatement_add"
        size="small"
        icon="el-icon-plus"
        type="primary"
        @click="handleAdd"
      ...
      >
      <!-- <el-button
        v-if="permission.purchaseStatement_audit"
        size="small"
      ...
      > -->
      <el-button
        v-if="permission.purchaseStatement_delete"
        size="small"
        icon="el-icon-delete"
        type="danger"
        plain
      ...
      >
      <el-button
        v-if="permission.purchaseStatement_printer"
        size="small"
        icon="el-icon-printer"
        plain
        @click="handlePrinter('AccountStatementV.1.3.05采购对账单.ureport')"
        >导出</el-button
      >
    </el-row>
    <el-row>
      ...
      >
        ...
        <el-table-column width="160" label="操作" fixed="right">
          <template slot-scope="scope">
            ...
            ></el-divider>
            <el-tooltip
              v-if="permission.purchaseStatement_edit"
              class="item"
              effect="dark"
              content="编辑"
              placement="bottom"
            ...
            ></el-divider>
            <el-tooltip
              v-if="permission.purchaseStatement_detail"
              class="item"
              effect="dark"
              content="详情"
              placement="bottom"
            ...
            <el-divider
              ...
              direction="vertical"
            ></el-divider>
            <span v-if="permission.purchaseStatement_delete">
              <el-tooltip
                class="item"
                effect="dark"
                content="删除"
...
export default {
  ...
  data() {
    return {
      ...
      pdfIdApi: "/erp/accountStatement/getPurchaseAccountBillsReportPDF",
      form: {
        accountStaSn: "",
        customerName: "",
        startDay: "",
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "编辑对账单" || from.name === "对账单详情") {
  ...
  methods: {
    ...
    handleAdd() {
      // 新增
      this.$router.push({
        path: "/purchaseManage/statement/add",
      });
    },
    handleEdit(row) {
      // 编辑
      this.$router.push({
        path: "/purchaseManage/statement/edit",
        query: {
          data: row,
        },
      });
    ...
    handleDetail(row) {
      // 详情
      this.$router.push({
        path: "/purchaseManage/statement/detail",
        query: {
          data: row,
        },
      });
