<template>
  <div>
    ...
    <el-row>
      <el-col :span="24"
        ...
        <el-button type="primary" @click="searchBtn" size="small"><i class="el-icon-search"></i>查询</el-button>
          ...
          <el-button class="btn-color" @click="RAudit" size="small">反审核</el-button>
    ...
    >
      ...
      <el-table-column prop="status" label="单据状态">
        <template v-slot:default="slotProps">
          ...
          <span v-if="slotProps.row.status == '2'">
            <el-tag color="#3399FF">采购</el-tag>
          </span>
          <span v-if="slotProps.row.status == '3'">
            <el-tag color="#999999">未入库</el-tag>
          </span>
          <span v-if="slotProps.row.status == '4'">
            <el-tag color="#3399FF">已入库</el-tag>
          </span>
          <span v-if="slotProps.row.status == '6'">
            <el-tag color="#FF5918">部分入库</el-tag>
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="purchaseMan" label="采购人"> </el-table-column>
      <el-table-column prop="taxRate" label="税率"> </el-table-column>
      <el-table-column prop="tax" label="税金"> </el-table-column>
      <el-table-column prop="totalAmount" label="总金额"> </el-table-column>
      <!-- 0 -->
      <el-table-column prop="" label="验收情况"> </el-table-column>
      <!-- 0 -->
      ...
      <el-table-column width="250" fixed="right" label="操作">
        <template v-slot:default="slotProps">
          <span class="span">
            <a @click="showPtance(slotProps.row)">验收</a
            ><a @click="showEdit('0', slotProps.row)">详情</a>
            <a @click="showEdit('1', slotProps.row)">编辑</a
            ><a @click="clearlist(slotProps.row)">删除</a>
            <a @click="showReturn(slotProps.row)">转退货</a></span
          >
          <!-- <span class="span">
            <a @click="showPtance(slotProps.row)">审核</a
            ><a @click="showEdit('0', slotProps.row)">详情</a>
            <a @click="showEdit('1', slotProps.row)">编辑</a
    ...
    <!-- 验收 -->
    <acce-ptance :detailAll="detailAllB" ref="ptance"></acce-ptance>

    <!-- 退货 -->
    <sales-return :detailAll='detailAllC' ref="return"></sales-return>
  </div>
</template>
<script>
...
export default {
  name: "",
  components: { topSeach, addEle, paginaEle, editEle, accePtance, salesReturn },
  props: {},
  data() {
    return {
        info:{
  ...
  methods: {
    ...
    // 验收
    async showPtance(ele) {
      const { data: res } = await getPurchaseOrderById({ id: ele.id });
      this.detailAllB = res.data;
      this.$refs.ptance.dialogVisible = true;
    },

    // 退货
    async showReturn(ele) {
      // const { data: res } = await getPurchaseOrderById({ id: ele.id });
      const { data: res } = await getPurchaseOrderById({ id: 26 });
      this.detailAllC = res.data;
      this.$refs.return.dialogVisible = true;
    },
    ...
    // 初始化数据
    async inittab() {
      const { data: res } = await getPurchaseOrderByPage();
    ...
    async updatepageNum(val) {
      const { data: res } = await getPurchaseOrderByPage(val);
      this.tableData = res.data.records;
      this.total = res.data.total;
    },
    ...
    // // 审核
    async Audit() {
      const { data: res } = await OrderStatus(this.array);
    ...
    // // 反审核
    async RAudit() {
      const { data: res } = await OrderStatusUn(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
      this.$message.success("操作成功");
      this.inittab();
    },
    ...
    async clearTab(val) {
      ...
      const { data: res } = await deleteOrderBatch(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
    ...
    async cancellAtion() {
      ...
      const { data: res } = await closeOrder(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
    ...
    myUpload(file) {
      ...
      xhr.open("POST", "/api/blade-rcm/equipmentPurchaseOrder/import", true);
    ...
    // 导出
    async infoexport() {
      const res = await closeOrderExport(this.array);
