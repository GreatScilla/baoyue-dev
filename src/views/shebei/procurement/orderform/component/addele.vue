...
export default {
  ...
  methods: {
   async open_all(){
     ...
     let customRes = await contactList({category:"1"})

     this.options=deptRes.data.data
     this.requestMan=userRes.data.data.records
     this.customMan=customRes.data.data
     console.log(customRes.data.data)
    },
    // // 添加
    async addForm() {
      this.mergeTab();
