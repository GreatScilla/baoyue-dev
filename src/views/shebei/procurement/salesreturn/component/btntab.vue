...
export default {
  ...
  methods: {
    ...
    // 初始化数据
    async inittab() {
      const { data: res } = await getPurchaseReturnInfoByPage();
    ...
    async updatepageNum(val) {
      const { data: res } = await getPurchaseReturnInfoByPage(val);
      this.tableData = res.data.records;
      this.total = res.data.total;
    },
    ...
    // 审核
    async Audit() {
      const { data: res } = await reviewPurchase(this.array);
    ...
    // 反审核
    async UnAudit() {
      const { data: res } = await UnreviewPurchase(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
      this.$message.success("操作成功");
      this.inittab();
    },
    ...
    async clearTab(val) {
      ...
      const { data: res } = await deletePurchaseReturnById(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
    ...
    async cancellAtion() {
      ...
      const { data: res } = await ReturncloseOrder(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
    ...
    myUpload(file) {
      ...
      xhr.open(
        "POST",
        "/api/blade-rcm/equipmentReturn/import",
        true
      );
    ...
    // 导出
    async infoexport() {
      const res = await equipmentReturn(this.array);
    ...
    async showEdit(val, ele) {
      const { data: res } = await ReturnOrderById({ id: ele.id });
