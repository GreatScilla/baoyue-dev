...
export default {
  ...
  methods: {
   async open_all(){
     ...
     let businessRes = await contactList({category:"1"})
      this.businessMan=businessRes.data.data
     console.log(businessRes.data.data)
     console.log(this.businessMan,5656)
     this.saleDept=deptRes.data.data
     this.saleMan=userRes.data.data.records
    },
    // // 添加
    async addForm() {
      this.mergeTab();
    ...
    // 合二为一
    mergeTab() {
      this.ruleForm.equipmentContractOrderItemList = [];
