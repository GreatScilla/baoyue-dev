  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="typeClass" label="入库类别">
        <template v-slot:default="slotProps">
          <span v-if="slotProps.row.typeClass == '0'">采购</span>
          <span v-else-if="slotProps.row.typeClass == '1'">采购退货</span>
          <span v-else-if="slotProps.row.typeClass == '2'">其他</span>
          <span v-else-if="slotProps.row.typeClass == '3'">零售</span>
          <span v-else-if="slotProps.row.typeClass == '4'">销售</span>
          <span v-else>调拔</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 初始 表格
    async pageInit() {
      const { data: res } = await pageTable({ type: "出库" });
      this.tableData = [];
    ...
    async updelAsset(val) {
      ...
      const { data: res } = await InfoisAsset(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
