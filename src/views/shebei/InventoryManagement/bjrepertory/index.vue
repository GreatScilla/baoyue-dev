      ...
      <el-row :gutter="24" class="Mtop">
        ...
        <el-col :span="20">
          ...
          >
            ...
            <el-table-column prop="alert_status" label="预警状态">
              <template v-slot:default="slotProps">
                <el-tag v-if="slotProps.row.alertStatus == '0'" color="#1ABC9C"
                  >正常</el-tag
                >
                <el-tag
                  v-else-if="slotProps.row.alertStatus == '1'"
                  color="#FF5918"
                  >高库存</el-tag
                >
                <el-tag v-else-if="slotProps.row.alertStatus == '2'" color="#F59A23">低库存</el-tag>
              </template>
            </el-table-column>
            <el-table-column
              prop="name"
...
import { pageList, bjexportList } from "@/api/shebei/InventoryManagement";
...
export default {
  ...
  methods: {
    ...
    async getPage() {
      // 分页
      const { data: res } = await pageList(this.paging);
      this.tableData = res.data.records;
    },

    // 导出
    async daochu() {
      const res = await bjexportList(this.array);
    ...
    async initQuery() {
      ...
      const { data: res } = await pageList(this.repertoryForm);
      this.tableData = res.data.records;
    },
