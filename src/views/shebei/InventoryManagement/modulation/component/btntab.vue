<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="makeStockName" label="盘点仓库"> </el-table-column>
      ...
      <el-table-column width="160" fixed="right" label="操作">
        <template v-slot:default="slotProps">
          <span class="span" v-if="slotProps.row.status == '0'"
            ...
            ><a @click="delList(slotProps.row)">删除</a></span
          >
          <span
            class="span"
            v-if="slotProps.row.status == '4'"
            ><a @click="showEdit('0', slotProps.row)">详情</a></span
          >
          <span
            class="span"
            v-if="slotProps.row.status == '1'"
            ><a @click="showEdit('2', slotProps.row)">调整</a><a @click="showEdit('0', slotProps.row)">详情</a></span
          >
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async updatepageNum(val) {
      const { data: res } = await ininData(val);
    ...
    // 审核
    async Audit() {
      const { data: res } = await updateSaleStatus(this.array);
    ...
    async delList(val) {
      ...
      const { data: res } = await stockHeadRemoveBean(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
    ...
    async cancellAtion() {
      ...
      const { data: res } = await stockHeadIsVoid(this.array);
      if (res.code !== 200) return this.$message.error(res.msg);
    ...
    // 导出
    async infoexport() {
      const res = await outExport(this.array);
    ...
    async showEdit(val, ele) {
      const { data: res } = await stockHeadDetail({ id: ele.id });
