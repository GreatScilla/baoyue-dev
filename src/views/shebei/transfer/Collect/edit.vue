...
<!--      </el-button>-->
      ...
      <b>·领用设备明细</b>
      <el-divider></el-divider>
      <div class="box" v-show="this.info.flag==1">
...
<script>
import {addCollect,collectDetail} from "@/api/shebei/transfer"
...
export default {
  ...
  async  saveEdit() {
   ...
   async open_all(){
     ...
     this.equipmentData=[]
     this.spareData=[]
        let res = await collectDetail({id:this.info.id})
     this.formInline={
       number:res.data.data.number,
       time:res.data.data.createTime,
       user:res.data.data.approvedBy,
       dept:res.data.data.dept,
       receiver:res.data.data.receiver
     }
     this.equipmentData=res.data.data.equipmentInfos
     this.spareData=res.data.data.equipmentInventories
     if(this.info.flag==0){
       setTimeout(()=>{
         this.$refs.formInline.disabled=true
    ...
    choiceAppendaspareBtn() {
      this.spareData = this.dataSpare;
      console.log(this.addSpareData)
    ...
    // 添加设备
    choiceAppendEqBtn() {
      this.equipmentData = this.dataEquipment,
        this.appendEq = false
    },
    //添加设备多选
    addEqSelectChange(row) {
      this.dataEquipment = row
    },
    //测试按钮(
    qwwe() {
    ...
    async spareBtn() {
      ...
      this.addspareData = res.data.data.records
      this.appendSpare = true
    },
    //添加设备按钮
    async appendEquip() {
      let res = await getShebei()
      this.addEquipmentData = res.data.data.records
      this.appendEq = true
    },
    //编辑按钮
    edit() {
