<template>
  <div>
    ...
    >
      ...
      <el-table-column label="编号" prop="tagName" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="position" label="所在位置" show-overflow-tooltip>
      </el-table-column>
      <el-table-column
        prop="temperatureTop"
        label="温度上限"
...
export default {
  ...
  methods: {
    ...
    async deletAll() {
      let res = await thermometerDelet({ ids: this.rowIds });
    ...
    async getpage(para) {
      let res = await thermometerPage(para);
    ...
    async delBtn(index, row) {
      let res = await thermometerDelet({ ids: row.id });
      if (res.data.code == 200) {
        this.$message({
          message: "删除成功",
          type: "success",
