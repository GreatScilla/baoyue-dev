  ...
  <basic-container>
    ...
    <el-row class="operation-function">
      <el-button
        ...
        v-if="permission.inbound_search"
        >查 询</el-button
      >
      <el-button
        size="small"
      ...
      <!-- <el-button
        ...
        plain
        @click="examine"
        v-if="permission.inbound_examine"
        >批量提交审核</el-button
      > -->
      <el-button
        size="small"
      ...
      >
      <!-- <el-button
        v-if="permission.inbound_printer"
        size="small"
        icon="el-icon-printer"
        plain
        @click="handlePrinter('JshDepotHeadV.1.0.0仓库入库单.ureport')"
        >导出</el-button
      > -->
      <!-- <el-button size="small" icon="el-icon-upload2" type="success" plain v-if="permission.inbound_export">导 出</el-button>
      <el-button size="small" icon="el-icon-printer" type="primary" plain v-if="permission.inbound_print">打 印</el-button>-->
    </el-row>
    ...
    >
      ...
      <el-form-item label-width="100px" label="入库类别:">
        <el-select size="small" v-model="searchForm.typeClassId"  placeholder="请选择入库类别" @change="changeTypeClass">
          <el-option
      ...
      <el-row>
        <el-form-item label="入库状态:">
          <el-select size="small" v-model="searchForm.status"  placeholder="请选择入库状态">
            <el-option
    ...
    <el-row>
      ...
      >
        ...
        <el-table-column
          ...
          width="150"
        ></el-table-column>
        <el-table-column prop="typeClass" label="入库类别"></el-table-column>
        <el-table-column prop="processTypeName" label="入库状态">
          <template slot-scope="scope">{{
        ...
        <el-table-column width="180" label="操作" fixed="right">
          <template slot-scope="scope">
            ...
            <el-divider
              direction="vertical"
              v-if="permission.inbound_edit && permission.inbound_details"
            ></el-divider>
            <el-tooltip
              class="item"
              effect="dark"
            ...
            >
              <el-button
                ...
                v-if="permission.inbound_details"
              ></el-button>
            </el-tooltip>
            <el-divider
              direction="vertical"
              v-if="permission.inbound_details && permission.inbound_delete"
            ></el-divider>
            <span v-if="permission.inbound_delete">
              <el-tooltip
                class="item"
                effect="dark"
                content="删除"
    ...
    >
      ...
      >
        ...
        <el-form-item label="入库类别">
          <el-select v-model="searchForm.typeClass" placeholder="请选择">
            <el-option
...
export default {
  ...
  data() {
    return {
      testDisabled: false,
      pdfIdApi: "/erp/depotHead/getWarehousingBillsReportPDF",
      searchForm: {},
      recordForm: {}, //暂存数据
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "编辑入库管理" || from.name === "入库管理详情") {
  ...
  methods: {
    ...
    async examine(row) {
      ...
      try {
        ...
        let res = await examineInAndOutBoundPage({ ids: ids, type: 1 });
        if (res.data.success) {
          this.handleList();
    ...
    changeTypeClass(value) {
      ...
      this.searchForm.typeClass = item[0].dictValue;
    },
    // 选择表格
    ...
    add() {
      this.$router.push("/storageManage/inboundManage/add");
    },
    // 跳转
    edit(data) {
      this.$router.push({
        path: "/storageManage/inboundManage/edit",
        query: {
          data,
        },
      });
    ...
    detail(data) {
      this.$router.push({
        path: "/storageManage/inboundManage/detail",
        query: {
          data,
        },
      });
