...
import addEle from "./popup/sbadd";
import editEle from "./popup/sbedit";
export default {
  name: "",
  ...
  methods: {
    ...
    // 查询
    async infoQuery() {
      const { data: res } = await deviceDatabase(this.form);
    ...
    async showEdit(val, ele) {
      const { data: res } = await detailEviceDatabase({ id: ele.id });
    ...
    async updatepageNum(val) {
      const { data: res } = await deviceDatabase(val);
      this.tableData = res.data.records;
      this.total = res.data.total;
    },
    ...
    // 初始化数据
    async ininTab() {
      const { data: res } = await deviceDatabase();
      this.tableData = res.data.records;
      this.total = res.data.total;
    },
    ...
    async delList(val) {
      ...
      const { data: res } = await removeEviceDatabase({ ids: val.id });
