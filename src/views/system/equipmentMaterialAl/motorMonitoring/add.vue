      ...
      <el-row>
        ...
        <el-row>
          <el-col :span="12" class="deatilsColumn">
            ...
            <el-form-item required label="电量下限" prop="electricQuantityLow">
              <span v-if="type === 'detail'">{{
                form.electricQuantityLow
              }}</span>
              <el-input v-else size="small" v-model="form.electricQuantityLow">
              </el-input>
            </el-form-item>
            <el-form-item required label="温度下限" prop="temLow">
              <span v-if="type === 'detail'">{{ form.temLow }}</span>
              <el-input v-else size="small" v-model="form.temLow"> </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12" class="deatilsColumn">
            ...
            <el-form-item required label="温度上限" prop="temTop">
              ...
              <el-input v-else size="small" v-model="form.temTop"> </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row style="margin-left: -35px">
...
import {
  ...
  submit,
  detailList,
} from "@/api/equipmentMaterialAl/motorMonitoring";
export default {
  props: {
    type: {
      type: String,
  ...
  methods: {
    async saveData() {
      ...
      if (this.paramPointList.length <= 0) {
        ...
        return;
      } else if (this.paramPointList.length < 2) {
        this.$message.error({
          message: "请选择两个参数点位",
        });
        return;
