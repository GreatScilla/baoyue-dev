...
<div>
  <el-dialog
    ...
    <el-row>
      <el-col :span="24"><div class="grid-content bg-purple-dark">
        <el-button type="primary" size="small" @click="addStandard">添加备件</el-button>
...
import {addMaintenancestandards} from "@/api/shebei/Maintenancestandards"
import  {saveCheckStandard} from "@/api/shebei/equipmentCheck"
export default {
  props:["info"],
  data() {
    return {
  ...
  methods: {
    handleSelChange(val){
      ...
      this.sparePartIds = newarr.join(",");
      const ired = val.map(ele => ele.how);
      this.quantityRequired=ired.join(",");
      console.log(this.quantityRequired)
      console.log(this.sparePartIds)
    },
    resetSpare(){
      this.standardForm={}
    },
    ...
    async saveAll(){
      let para = {
        ...
        maintenanceParts:this.formInline.maintenanceParts,
        maintenanceCycle:this.formInline.how+this.formInline.region,
        maintenanceMethod:this.formInline.maintenanceMethod,
        sparePartIds:this.sparePartIds,
        quantityRequired:this.quantityRequired,
        memo:this.formInline.memo,
      }
      console.log(para)
    let res = await addMaintenancestandards()
    },
