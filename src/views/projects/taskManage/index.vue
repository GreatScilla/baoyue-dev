...
<template>
  <basic-container>
    ...
    >
      ...
      <el-form-item label-width="100px" label="项目阶段:">
        <el-select size="small" v-model="mesClassId" placeholder="请选择项目阶段" @change="changeShift">
          <el-option
    ...
    <el-row style="margin-bottom: 20px;">
      ...
      <el-button size="small" icon="el-icon-delete" type="danger" plain @click="handleDelInboundManage">删除</el-button>
      <el-button size="small" icon="el-icon-upload2" type="success" plain>导入</el-button>
      ...
      <el-button size="small" icon="el-icon-coordinate" type="primary" plain @click="examine">作废</el-button>
    </el-row>
    <el-row>
      <el-table
      ...
      >
        ...
        <el-table-column prop="procCode" label="目标阶段">
          <template slot-scope="scope">
            {{scope.row.status | billStatus}}
          </template>
        </el-table-column>
        <el-table-column prop="procName" label="任务名称">
          <template slot-scope="scope">
            <el-image
              v-if="scope.row.file"
        ...
        <el-table-column prop="number" label="任务编码">
        </el-table-column>
         <el-table-column prop="createTime" label="关联项目" width="150">
        </el-table-column>
        <el-table-column prop="createUser" label="任务类型">
        </el-table-column>
        <el-table-column prop="checkPerson" label="开始时间">
        </el-table-column>
        <el-table-column prop="checkPerson" label="结束时间">
        </el-table-column>
        <el-table-column prop="checkPerson" label="负责人">
        </el-table-column>
        <el-table-column prop="checkPerson" label="项目阶段">
        </el-table-column>
        <el-table-column prop="checkPerson" label="优先级">
        </el-table-column>
        <el-table-column prop="checkPerson" label="任务描述">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    to(operationState,row) {
      this.$router.push({
        path: "/projects/taskManage/detail",
        query: {
          operationState,
          row
        }
