...
<template>
  <basic-container>
    ...
    >
      ...
      >
        ...
        <el-form-item label="报工状态:">
          <el-select
            size="small"
            v-model="searchForm.outletArtisanStatus"
            placeholder="请选择"
          >
            <el-option
        ...
        <el-form-item label="撤销状态:">
          <el-select
            size="small"
            v-model="searchForm.outletRepealStatus"
            placeholder="请选择"
          >
            <el-option
    ...
    <el-row style="margin-bottom: 20px">
      <el-button
        v-if="permission.IntoKettleWorkOrder_expore"
        size="small"
        icon="el-icon-search"
        type="primary"
        ...
        >查 询</el-button
      >
      <!-- <el-button  v-if="permission.IntoKettleWorkOrder_delete" size="small" @click="handleDelete()"> 批量删除 </el-button>
      <el-button
       v-if="permission.IntoKettleWorkOrder_export"
      ...
      >
        ...
        <el-table-column label="报工状态">
          <template slot-scope="scope">
            {{ scope.row.outletArtisanStatus | statusName }}
          </template>
        </el-table-column>
        <el-table-column label="撤销状态">
          <template slot-scope="scope">
            {{ scope.row.outletRepealStatus | repealName }}
          </template>
        </el-table-column>
        ...
        <el-table-column width="60" label="操作" fixed="right">
          <template slot-scope="scope">
            <router-link
              v-if="permission.IntoKettleWorkOrder_detail"
              :to="{
                path: '/production-management/outKettleWorkOrder/detail',
            ...
            <!-- <el-tooltip
              v-if="permission.IntoKettleWorkOrder_delete"
              class="item"
              effect="dark"
              content="删除"
              placement="bottom"
            >
              <el-button
                ...
                :disabled="scope.row.outletArtisanStatus != 0"
              ></el-button>
            </el-tooltip>
            <el-divider direction="vertical"></el-divider> -->
            <!-- <el-tooltip
              v-if="permission.IntoKettleWorkOrder_putSeal"
              class="item"
              effect="dark"
              content="打印"
              placement="bottom"
...
import {
  getOutletWorkOrderByPage,
  deleteIrigamaWorkOrder,
  getIrigamaWorkOrderExport,
} from "@/api/manufacture/outKettleWorkOrder";
...
export default {
  ...
  data() {
    return {
      ...
      exportApi: getIrigamaWorkOrderExport,
      pdfIdApi: "/mes/irigamaWorkOrder/getIrigamaWorkOrderReportPDF",
      ids: [], //id
      teamList: [],
  ...
  methods: {
    ...
    async handleList(...arg) {
      ...
      const res = await getOutletWorkOrderByPage(params);
