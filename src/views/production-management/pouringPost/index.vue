  ...
  <basic-container>
    ...
    <el-row style="margin: 10px 0">
      ...
      <el-button
        ...
        icon="el-icon-plus"
        type="primary"
        @click="$router.push('/production-management/pouringPost/add')"
        >新 增</el-button
      >
    </el-row>
    <el-row>
      ...
      >
        ...
        <el-table-column width="130" label="操作" fixed="right">
          <template slot-scope="scope">
            <router-link
              :to="{
                path: '/production-management/pouringPost/edit',
            ...
            <router-link
              :to="{
                path: '/production-management/pouringPost/detail',
                query: { data: scope.row },
              }"
            >
              <el-tooltip
...
import {
  pouringPostPage,
  pouringPostDel,
  getPourPositionRecordReportPDF,
...
export default {
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      pdfIdApi: getPourPositionRecordReportPDF,
    };
  },
  computed: {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "修改浇注中控岗位记录表" || from.name === "浇注中控岗位记录表详情") {
  ...
  methods: {
    ...
    async handleList() {
      ...
      const res = await pouringPostPage(params, this.searchForm);
