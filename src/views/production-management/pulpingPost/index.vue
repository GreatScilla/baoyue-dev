  ...
  <basic-container>
    ...
    <el-row style="margin: 10px 0">
      ...
      <el-button
        ...
        icon="el-icon-plus"
        type="primary"
        @click="$router.push('/production-management/pulpingPost/add')"
        >新 增</el-button
      >
    </el-row>
    <el-row>
      ...
      >
        ...
        <el-table-column width="130" label="操作" fixed="right">
          <template slot-scope="scope">
            <router-link
              :to="{
                path: '/production-management/pulpingPost/edit',
            ...
            <router-link
              :to="{
                path: '/production-management/pulpingPost/detail',
                query: { data: scope.row },
              }"
            >
              <el-tooltip
...
import {
  pulpingPostPage,
  pulpingPostDel,
  getPulpingPositionRecordReportPDF,
...
export default {
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      pdfIdApi: getPulpingPositionRecordReportPDF,
    };
  },
  computed: {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "修改制浆岗位操作记录" || from.name === "制浆岗位操作记录详情") {
  ...
  methods: {
    ...
    async handleList() {
      ...
      const res = await pulpingPostPage(params, this.searchForm);
