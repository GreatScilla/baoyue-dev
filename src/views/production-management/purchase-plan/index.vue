...
<template>
  <basic-container>
    ...
    <el-row style="margin: 20px 0">
      <el-button
        v-if="permission.purchasePlan_expore"
        size="small"
        icon="el-icon-search"
      ...
      >
      <!-- <el-button
        v-if="permission.purchasePlan_audit"
        plain
        size="small"
        @click="handleAudit()"
        >批量提交审核</el-button
      > -->
      <el-button
        v-if="permission.purchasePlan_delete"
        size="small"
        @click="handleDelete()"
        >批量删除
      </el-button>
      <!-- <el-button
        v-if="permission.purchasePlan_printer"
        size="small"
        icon="el-icon-printer"
        plain
        @click="handlePrinter('MesProcurementPlanV1.1.2采购申请单.ureport')"
        >导出</el-button
      > -->
    </el-row>
    <el-row>
      ...
      >
        ...
        <!-- <el-table-column
          ...
          label="采购数量"
        ></el-table-column> -->
        <el-table-column prop="dateOrder" label="到货日期"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right">
          <template slot-scope="scope">
            <span v-if="permission.purchasePlan_audit">
              <el-tooltip
                class="item"
                effect="dark"
                content="提交审核"
            ...
            <span v-if="permission.purchasePlan_edit">
              <el-tooltip
                class="item"
                effect="dark"
                content="编辑"
              ...
              >
                <el-button
                  ...
                  :disabled="scope.row.documentsState === '1'"
            ...
            <router-link
              v-if="permission.purchasePlan_detail"
              :to="{
                path: '/production-management/purchase-plan/detail',
            ...
            <span v-if="permission.purchasePlan_delete">
              <el-tooltip
                class="item"
                effect="dark"
                content="删除"
...
export default {
  ...
  data() {
    return {
      searchDate: "",
      pdfIdApi: "/mes/procurementplan/getProcurementPlanBillsReportPDF",
      searchForm: {},
      recordForm: {}, //暂存数据
      tableData: [],
      options: [
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "编辑采购计划" || from.name === "采购计划详情") {
  ...
  methods: {
    ...
    toEdit(data) {
      this.$router.push({
        path: "/production-management/purchase-plan/edit",
        query: { data: data },
      });
    },
    //删除角色
    async handleDelete(row) {
      ...
      try {
        ...
        ids = ids.join(",");
        const res = await remove({ procurementPlanIds: ids });
        if (res.data.code === 200) {
          this.$message.success("操作成功");
          await this.handleList();
        }
