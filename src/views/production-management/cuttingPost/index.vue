  ...
  <basic-container>
    ...
    <el-row style="margin: 10px 0">
      ...
      <el-button
        ...
        icon="el-icon-plus"
        type="primary"
        @click="$router.push('/production-management/cuttingPost/add')"
        >新 增</el-button
      >
    </el-row>
    <el-row>
      ...
      >
        ...
        <el-table-column width="130" label="操作" fixed="right">
          <template slot-scope="scope">
            <router-link
              :to="{
                path: '/production-management/cuttingPost/edit',
            ...
            <router-link
              :to="{
                path: '/production-management/cuttingPost/detail',
                query: { data: scope.row },
              }"
            >
              <el-tooltip
...
import {
  cuttingPostPage,
  cuttingPostDel,
  getCuttingPositionRecordReportPDF,
...
export default {
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      pdfIdApi: getCuttingPositionRecordReportPDF,
    };
  },
  computed: {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === "修改切割岗位记录表" || from.name === "切割岗位记录表详情") {
  ...
  methods: {
    ...
    async handleList() {
      ...
      const res = await cuttingPostPage(params, this.searchForm);
